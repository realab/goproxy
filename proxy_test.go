...
func TestSimpleHttpRequest(t *testing.T) {
	// Local origin server so the test doesn't depend on external hosts.
	...
	// Use an OS-assigned port via a listener instead of a hardcoded port.
	...
	go func() {
		if err := server.Serve(ln); err != nil && err != http.ErrServerClosed {
			t.Error("Unexpected server error", err)
		}
	}()
	...
	resp, err := client.Do(req)
	if err != nil {
		t.Fatal("Error requesting http site", err)
	} else if resp.StatusCode != http.StatusOK {
		t.Error("Non-OK status requesting http site")
	}
	_ = resp.Body.Close()
...
func connectViaProxy(ctx context.Context, proxyAddr, target string) (net.Conn, error) {
	...
	return &readBufferedConn{Conn: conn, r: br}, nil
}

// readBufferedConn wraps a net.Conn with a buffered reader, identical to the
